...
# 
# #filter to areas within study area. Then add E/W designation
...
# 
# #Save all species names, including subspecies reported
...
cbc_core_w_cell_filt <- readRDS("data/2_formatted/Irruptions/cbc_core_w_cell_filt.rds")

wint_season_range_all <- wint_season_range

for (each_tm_year in 1:21){
  
  wint_season_range <- wint_season_range_all[1:(length(wint_season_range_all)-each_tm_year)]
  ...
  write_csv(all_spec_frame,paste("data/2_formatted/Irruptions/time_machine/irruptions_detrended_east_tm_",each_tm_year,".csv",sep=""))
  ...
  write_csv(all_spec_frame,paste("data/2_formatted/Irruptions/time_machine/irruptions_detrended_west_tm_",each_tm_year,".csv",sep=""))
}
